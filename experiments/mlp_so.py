...
def inference(input_tensor_batch):
    ...
    with tf.name_scope('hidden3'):
        hidden3 = tf.nn.relu(bn_layer(fc_layer(hidden2, 32)))

    with tf.name_scope('softmax_linear'):
        logits = fc_layer(hidden3, 10)
...
# ===================================================================
tf.app.flags.DEFINE_float("p1", 0.01, "p1.")
tf.app.flags.DEFINE_float("p2", 0.005, "p2.")
tf.app.flags.DEFINE_float("p3", 0.001, "p3.")
tf.app.flags.DEFINE_float("r", 1e-4, "r.")
...
def main(argv):
    ...
    with tf.Session() as sess:
        ...
        # df.to_csv("./logs/csv/mlp_so_({})_({})_({})_({})_({}).csv".format(
        #     FLAGS.p1, FLAGS.p2, FLAGS.p3, FLAGS.r, FLAGS.s))
        df.to_csv("./logs/csv/mlp_so.csv")
