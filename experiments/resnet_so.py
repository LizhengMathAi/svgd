...
def conv_layer(input_tensor, filter_shape, stride, transpose=False):
    ...
    if transpose:
        ...
        output_tensor = ops.conv2d(relu_tensor, kernel, strides=[1, stride, stride, 1], padding="SAME")
    else:
        assert input_tensor.get_shape().as_list()[-1] == filter_shape[2]
        ...
        conv2d_tensor = ops.conv2d(input_tensor, kernel, strides=[1, stride, stride, 1], padding='SAME')
...
def residual_block(input_tensor, output_channel, first_block=False):
    ...
    with tf.name_scope('conv_1'):
        if first_block:
            ...
            kernel = tf.Variable(initial_value=kernel_init, dtype=tf.float32, name="kernel")
            tf.add_to_collection(tf.GraphKeys.REGULARIZATION_LOSSES, tf.reduce_sum(tf.abs(kernel)))
            conv1 = ops.conv2d(input_tensor, kernel, strides=[1, 1, 1, 1], padding="SAME")
        else:
...
tf.app.flags.DEFINE_float("p1", 0.5, "p1.")
tf.app.flags.DEFINE_float("p2", 0.02, "p2.")
tf.app.flags.DEFINE_float("p3", 0.01, "p3.")
tf.app.flags.DEFINE_float("s", 0.01, "s.")
...
def main(argv):
    ...
    with tf.Session() as sess:
        ...
        df.to_csv("./logs/csv/resnet_so_({})_({})_({})_({}).csv".format(FLAGS.p1, FLAGS.p2, FLAGS.p3, FLAGS.s))
